...
const COURSES_URL = "https://hishtmby.org.il"; // החליפי לכתובת הנכונה
...
const PUZZLE = {
  ...
  groups: [
    ...
    },
    {
      ...
      hint2:
        "בעתודה תקבלי הכשרה מקצועית שפותחת דלתות - תעודה, התמחות, ודעה מקצועית!",
    },
  ],
};
...
export default function App() {
  ...
  const handleCheck = () => {
    ...
    if (match) {
      ...
      setGuessHistory((h) => [
        ...
        {
          colors: selected.map(
            (w) => puzzle.groups.find((g) => g.words.includes(w)).color
          ),
          correct: true,
        },
      ]);
      if (ns.length === NUM_GROUPS)
        setTimeout(() => {
          setWon(true);
          setGameOver(true);
        }, 800);
    } else {
      const ng = guessesLeft - 1;
      setGuessesLeft(ng);
      ...
      if (
        puzzle.groups.some(
          (g) =>
            !solved.includes(g) &&
            selected.filter((w) => g.words.includes(w)).length ===
              GROUP_SIZE - 1
        )
      ) {
        setAlmostMsg(true);
        setTimeout(() => setAlmostMsg(false), 2500);
      }
  ...
  const handleShuffle = () => {
    ...
    setWords([...sv, ...shuffle(uv)]);
    setShuffleKey((k) => k + 1);
  };

  const handleShare = async () => {
    const em = {
      [puzzle.groups[0].color]: "🟢",
      [puzzle.groups[1].color]: "🔵",
      [puzzle.groups[2].color]: "🔴",
      [puzzle.groups[3].color]: "🟡",
      [puzzle.groups[4].color]: "🟣",
    };
    const rows = guessHistory
      .map((g) => g.colors.map((c) => em[c] || "⬜").join(""))
      .join("\n");
    const hi = hintsUsed > 0 ? ` (💡${hintsUsed})` : "";
    const text = `✨ חידת קשרים — עתודה ✨\n${
      won
        ? `פתרתי ב-${guessHistory.length} ניחושים${hi}!`
        : "לא הצלחתי הפעם 😅"
    }\n\n${rows}\n\nנסי גם → ${COURSES_URL}`;
  ...
  const handleCoursesClick = () => {
    window.location.href = COURSES_URL;
  };
      ...
      <div style={S.page}>
        <div style={S.welcomeCenter}>
          <div style={S.welcomeCard} className="fade-in">
            ...
            <div style={S.wLine} />
            <p style={S.wDesc}>
              מצאי את הקשר הנסתר בין המילים
              <br />
              5 קבוצות · 15 מילים · קשר אחד
            </p>
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === "Enter" && playerName.trim() && setScreen("game")
              }
              style={S.wInput}
              dir="rtl"
            />
            ...
            <button onClick={() => setShowHelp(true)} style={S.wLink}>
              איך משחקים?
            </button>
          </div>
        </div>
  ...
  return (
    <div style={S.page}>
      <div style={S.header}>
        ...
        <p style={S.subtitle}>
          שלום {playerName} 👋 מצאי 5 קבוצות של 3 מילים
        </p>
      </div>
      ...
      <div style={S.boardWrap}>
        {solved.map((g) => (
          <div
            key={g.category}
            className="fade-in"
            style={{ ...S.solvedBar, backgroundColor: g.color }}
          >
        ...
        {currentHint && (
          <div
            className="fade-in"
            style={{ ...S.hintBox, borderRightColor: currentHint.color }}
          >
            {currentHint.type === "marketing" && (
              <div style={S.hintCatRow}>
                <span
                  style={{ ...S.hintDot, background: currentHint.color }}
                />
                הקטגוריה: {currentHint.category}
              </div>
            )}
            <p style={S.hintText}>{currentHint.text}</p>
            {currentHint.type === "marketing" && (
              <span style={S.hintTag}>💜 עתודה</span>
            )}
          </div>
        )}

        {almostMsg && (
          <div className="fade-in" style={S.almost}>
            כמעט! חסרה לך מילה אחת 🫣
          </div>
        )}
      </div>
            ...
            {Array.from({ length: MAX_GUESSES }).map((_, i) => (
              <span
                ...
                style={{
                  ...S.dot,
                  background:
                    i < guessesLeft ? "#58B8A0" : "rgba(255,255,255,0.15)",
                }}
              />
            ))}
          </div>
          ...
          <div style={S.dotsRow}>
            {Array.from({ length: MAX_HINTS }).map((_, i) => (
              <span
                ...
                style={{
                  ...S.hintIndicator,
                  opacity: i < MAX_HINTS - hintsUsed ? 1 : 0.2,
                }}
              >
                💡
              </span>
      ...
      <div style={S.actions}>
        <button onClick={handleShuffle} style={S.btn}>
          🔀 ערבבי
        </button>
        <button
          onClick={() => setSelected([])}
          disabled={!selected.length}
          style={{ ...S.btn, opacity: selected.length ? 1 : 0.3 }}
        >
          ↩ נקי
        </button>
        <button
          ...
          style={{
            ...S.btnHint,
            opacity: hintsUsed >= MAX_HINTS || gameOver ? 0.3 : 1,
          }}
        >
          💡 רמז
        </button>
        <button
          ...
          style={{
            ...S.btnCheck,
            opacity: selected.length === GROUP_SIZE ? 1 : 0.3,
          }}
        >
          בדיקה ✓
        </button>
      ...
      <button onClick={() => setShowHelp(true)} style={S.helpLink}>
        ? איך משחקים
      </button>

      <div style={S.footer}>
        <span style={S.footerBrand}>עתודה</span> · לימודי תעודה ומקצוע · מרכז בית
        יעקב
      </div>
      ...
      {gameOver && (
        <div style={S.overlay}>
          <div style={S.modal} className="fade-in">
            ...
            <p style={S.mText}>
              {won
                ? `פתרת ב-${guessHistory.length} ניחושים${
                    hintsUsed > 0 ? ` (${hintsUsed} רמזים)` : ""
                  }`
                : `לא נורא ${playerName}, כל ניסיון מחכים!`}
            </p>
            ...
            <div style={S.resGrid}>
              {guessHistory.map((g, i) => (
                <div key={i} style={S.resRow}>
                  {g.colors.map((c, j) => (
                    <span key={j} style={{ ...S.resDot, background: c }} />
                  ))}
                </div>
              ))}
            </div>

            <button onClick={handleShare} style={S.modalBtn}>
              שתפי תוצאות 📤
            </button>
            ...
            <div style={S.ctaBox}>
              ...
              <button onClick={handleCoursesClick} style={S.ctaBtn}>
                גלי את הקורסים שלנו →
              </button>
            </div>

            <button onClick={initGame} style={S.mLink}>
              שחקי שוב 🔄
            </button>
          </div>
        </div>
    ...
    <div style={S.overlay} onClick={onClose}>
      <div style={S.modal} className="fade-in" onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ textAlign: "right", lineHeight: 2.2, fontSize: 14, color: "#555" }}>
          <p>
            לפנייך <strong>15 מילים</strong> המחולקות ל-<strong>5 קבוצות</strong> של 3 מילים עם קשר משותף.
          </p>
          <p style={{ marginTop: 12 }}>
            <strong>כללים:</strong>
            <br />
            • בחרי 3 מילים ולחצי "בדיקה"
            <br />
            • ניחוש נכון - הקבוצה תתגלה
            <br />
            • ניחוש שגוי - תפסידי ניחוש
            <br />
            • צדקת ב-2 מתוך 3 - תקבלי "כמעט!"
            <br />
            • 6 ניחושים · 2 רמזים
          </p>
          <p style={{ marginTop: 12 }}>
        ...
        <button onClick={onClose} style={{ ...S.modalBtn, marginTop: 20 }}>
          הבנתי! 👍
        </button>
      </div>
    </div>
...
const S = {
  page: {
    direction: "rtl",
    minHeight: "100vh",
    background: "linear-gradient(170deg, #1a1250 0%, #241868 50%, #1e1458 100%)",
  ...
  wIcon: {
    ...
    height: 56,
    borderRadius: 16,
    background: "linear-gradient(135deg, #302070, #4a35a0)",
    color: "#fff",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    fontSize: 24,
    margin: "0 auto 16px",
    boxShadow: "0 4px 16px rgba(48,32,112,0.3)",
  },
  ...
  tile: {
    ...
    borderRadius: 14,
    border: "none",
    background: "rgba(88, 100, 180, 0.35)",
  ...
  btnHint: {
    ...
    padding: "11px 6px",
    borderRadius: 12,
    border: "1px solid rgba(88,184,160,0.25)",
    background: "rgba(88,184,160,0.12)",
    fontSize: 13,
    fontWeight: 600,
    cursor: "pointer",
    fontFamily: "'Heebo', sans-serif",
  ...
  btnCheck: {
    ...
    color: "#fff",
    transition: "all 0.15s",
    boxShadow: "0 3px 14px rgba(88,184,160,0.3)",
  },
  ...
  modalBtn: {
    ...
    fontFamily: "'Heebo', sans-serif",
    marginBottom: 8,
    boxShadow: "0 4px 16px rgba(48,32,112,0.2)",
  },
  ...
  ctaBtn: {
    ...
    cursor: "pointer",
    fontFamily: "'Heebo', sans-serif",
    boxShadow: "0 3px 12px rgba(88,184,160,0.25)",
  },
};
